...
from utils.swim_evaluator import SWIMEvaluator
...
from dataset.swimdataset import *
...
def parse_args():
    ...
    parser.add_argument('-d', '--dataset', type=str,
                        default='SWIM', help='COCO, VOC and SWIM dataset')
    ...
    parser.add_argument('--eval_interval', type=int,
                            default=1, help='interval epoch between evaluations')
    parser.add_argument('-c', '--checkpoint', type=str,
                        help='pytorch checkpoint file path')
    parser.add_argument('--save_dir', type=str,
                        default='weights/enhanced',
...
def main():
    ...
    if args.dataset == 'COCO':
        dataset = COCODataset(
                  data_dir='/media/hkuit164/Sherry/ASFF/data/COCO/',
                  img_size=input_size,
        ...
        dataset = VOCDetection(root='/media/hkuit164/Sherry/ASFF/data/VOC',
                 image_sets = train_sets,
                 input_dim = input_size,
                 preproc=TrainTransform(rgb_means=(0.485, 0.456, 0.406),std=(0.229, 0.224, 0.225),max_labels=30))
        num_class = 20
    
    elif args.dataset == 'SWIM':
        train_sets = [('ENHANCE1', 'trainval')]
        dataset = SWIMDetection(root='./data/SWIM',
                 image_sets = train_sets,
                 input_dim = input_size,
                 preproc=TrainTransform(rgb_means=(0.485, 0.456, 0.406),std=(0.229, 0.224, 0.225),max_labels=30))
        num_class = 1
    else:
        print('Only COCO,VOC and SWIM datasets are supported!')
        return
    ...
    while epoch < epochs+1:
        ...
        # learning rate scheduling (cos or step)
            ...
            if iter_i % 1 == 0 and save_to_disk:
            # logging
                end = time.time()
